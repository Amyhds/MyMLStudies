...
class Model():
    def __init__(self):
        ...
        # 네트워크 (은닉층 -> 3)
        self.fc1 = tf.layers.dense(self.x_normalize, 128, activation=tf.nn.relu)
        self.fc2 = tf.layers.dense(self.fc1, 256, activation=tf.nn.relu)
        self.fc3 = tf.layers.dense(self.fc2, 128, activation=tf.nn.relu)
        self.out = tf.layers.dense(self.fc3, num_label, activation=tf.nn.softmax)
...
class ANN():
    ...
    def Write_Summray(self, accuracy, loss, accuracy_val, loss_val, batch):
        self.Summary.add_summary(self.sess.run(self.Merge, feed_dict={self.summary_loss: loss, 
                                                                      self.summary_acc: accuracy,
                                                                      self.summary_loss_val: loss_val, 
                                                                      self.summary_acc_val: accuracy_val}), batch)
...
if __name__ == '__main__':
    ...
    for epoch in range(num_epoch):
        ...
        for batch_idx in range(0, x_train.shape[0], batch_size):
            loss, accuracy, loss_val, accuracy_val = ann.train_model(train_x, train_y, batch_idx)
            ...
            ann.Write_Summray(accuracy, loss, accuracy_val, loss_val, batch_num)
    ...
    # 테스트 수행 
    loss_test, accuracy_test = ann.test_model(x_test, y_test_onehot)
    print('----------------------------------')
